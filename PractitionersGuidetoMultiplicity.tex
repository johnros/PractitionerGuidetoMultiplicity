...
% Is another FWER example needed?
...
%Where $R$ is the number of SNPs declared associated with any voxel, $V$ the number of such false declarations, $R_g$ the number of voxels declared associated with SNP $g$ and $V_g$ the number of such false declarations.
%
%$H^0_{b,g}$ for $b \in B$ and $g \in G$ be the hypothesis of no association between voxel $v$ and SNP $g$. Also, let $H^0_g$ be the hypothesis of no association between \emph{any} voxel and SNP $g$: $ H^0_g:= \underset{b\in B}{\cap} H^0_{b,g}$. Accordingly define $T_{b,g}=1$ if $H^0_{b,g}$ is true and 0 otherwise, $T_{g}=1$ if $H^0_{g}$ is actually true and 0 otherwise, and  $R_{b,g}=1$ if it is rejected. 



\subsection{\label{eg:electrode_selection}Electrode Selection in EEG}
% Not sure I should include this example. 
