...
We demonstrate this on the GWAS example from Section~\ref{sec:GWAS}. 
Also, in the spirit of \citet[sec.5]{scott_exploration_2006}, we introduce a possible financial consideration: selected SNPs will be sent for further investigation, so they can possibly be selected on the basis of future expected expenses, which are incurred if no association is found in the followup investigation.
We emphasize that the computation of the expected expenses is data driven, and refer to \cite{scott_exploration_2006} for details.

Consider the following four scenarios:

\begin{scenario}
A researcher searching for significant associations, concerned with the proportion of experiments that will lead him to a false finding on any particular SNP.
\label{e.g.:marginal_noSelection}
\end{scenario}

\begin{scenario}
A researcher searching for significant associations, concerned with the proportion of experiments that will lead him to a false finding on any combination of SNPs.
\label{e.g.:simultanous_noSelection}
\end{scenario}

\begin{scenario}
A researcher searching for significant associations, concerned with the proportion of experiments that will lead him to a false finding on any on any particular SNP which has showed low expected expenses.
\label{e.g.:marginal_Selection}
\end{scenario}

\begin{scenario}
A researcher searching for significant associations, concerned with the proportion of experiments that will lead him to a false finding on any combination of SNPs which have showen low expected expenses.
\label{e.g.:simultanous_Selection}
\end{scenario}


For clarification Table~\ref{tab:simultanous_vs_selective} offers their classification with respect to two criteria: 
(a) Is there a preliminary selection stage? In this case, selecting SNPs according to an expected loss.
(b) Are errors accounted per finding or per combination of findings? In this case, SNP or per combination of SNPs.
...
These distinctions are of practical importance as they immediately suggest the appropriate measure of error. 
...
Scenario~\ref{e.g.:simultanous_noSelection} is a classic simultaneous inference problem which calls for strong FWER control.

Scenario~\ref{e.g.:marginal_Selection} is a selective-inference problem.  Strong FWER control is a possible error measure; as it protects from \emph{any} combination of false findings, it naturally protects from the particular combination which passed the selection stage. This, however, is a conservative approach: why protect from all possible combinations if we only need protection from the combinations of selected SNPs\footnote{ This is analogous to the over-protection of using a Scheffe correction in ANOVA, which offers protection against all possible contrasts, even if we consider only a small subset of possible contrasts. See [find citation] for details.}? 
FDR or FDR as good error measures for this scenario, although the exact procedure to be used needs to be tailored for the particular selection used. [cite example?]

Scenario~\ref{e.g.:simultanous_Selection} is a simultaneous selective-inference problem.  
Requiring \emph{simultaneous} inference sets the bar, which is satisfied only by FWER. 
The presence of the selection, however, makes most off-the-shelf FWER controlling procedures overly conservative, so that tailoring the procedure for the particular selection in use might provide a power gain. [cite example?]

\begin{remark}
The financial aspect of the selection mechanism is immaterial. It was merely selected to demonstrate a data-driven selection stage preceding a hypothesis testing stage.
\end{remark}

\begin{remark}
The concepts of simultaneous versus selective inference, is typically demonstrated on post-selection confidence intervals~\cite{benjamini_false_2005}. We did not use this type of example as we wished to focus on testing problems. 
\end{remark}
