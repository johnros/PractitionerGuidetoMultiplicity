...
In order to protect himself from an uncontrolled number of erroneous findings, a researcher has to consider of the type of errors he wishes to avoid and select the adequate procedure for that particular error type and data structure. 
...
In this example, the researcher has seemingly already selected the desired error measure to control for-- FDR. It is actually the grouping families that are unclear. Several options can be considered:
\begin{enumerate}
\item[(a)] FDR control over all genetic associations found (regardless of the experiment)? %$\{genetics \; variantes\} X \{experimantes\}$
\item[(b)] FDR control over all genetic associations found within each experiment? Possibly, with no control overall? [TODO]
\end{enumerate}•
...
Specifying the expected deviation from the null for each hypothesis tested is no easy task. There exist however, several procedures which use the multitude of hypotheses tested in an attempt to empirically characterise the deviations from the null~\footnote{Under mild assumptions regarding the form these deviations might take.}, and harness this information to gain power. Essentially all rely on estimators of the probability of a hypothesis being a true null given the value of some test statistic $z_i$. This is the posterior probability of the null, also named ``local fdr'' and denoted by $fdr(z_i)$. The details can be found in \cite{efron_microarrays_2008}. 
This magnitude-- the probability of being null given the data-- is not an error rate but rather a test statistic. It is however a rather intuitive test statistic. So much so, that many authors, set the rejection criterion to $fdr(z_i)\leq 0.2$. Which lends itself to the interpretation that results with frequencies smaller than 1 in 5, under the null assumption, are ``dangerously prone to wasting investigators’ resources''~\cite{efron_microarrays_2008}. 

While intuitive and powerful, a researcher interested in protection from a particular error type, the relation between the $fdr(z_i)\leq 0.2$ criterion to the error rate is unclear.[TODO]
